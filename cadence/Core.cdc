pub contract Core {
  ...
  pub resource interface Public {
    ...
    pub fun asReadOnly(): Core.ReadOnly
  }
  ...
  pub resource interface Owner {
    ...
    pub fun new(): @Core.Base {
        return <- create Base()
    }

    pub fun check(_ address: Address): Bool {
        return getAccount(address)
            .getCapability<&{Core.Public}>(Core.publicPath)
            .check()
    }

    pub fun fetch(_ address: Address): &{Core.Public} {
        return getAccount(address)
            .getCapability<&{Core.Public}>(Core.publicPath)
            .borrow()!
    }

    pub fun read(_ address: Address): Core.ReadOnly? {
        if let profile = getAccount(address).getCapability<&{Core.Public}>(Core.publicPath).borrow() {
